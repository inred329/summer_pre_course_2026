...
本章活動不要求繳交。建議自行保留預測、程式、錯誤與修正結果，供之後複習或課堂討論使用。
...
不需要使用固定 Prompt，也不需要保存或繳交對話。這個活動只是讓你有機會把理解說出來，並觀察對話是否讓你發現還不清楚的地方。

AI 的回應仍可能不完整或錯誤。若它的說法和本章流程圖、編譯結果或實際執行現象不一致，請以可重現的操作與證據重新判斷，而不是直接把 AI 回應當成答案。
