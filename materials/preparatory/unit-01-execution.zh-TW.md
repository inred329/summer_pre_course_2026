...
本文件是提供學生自行閱讀、操作、複習與查閱的教學教材。它不是教師授課排程、評量規則或作業繳交說明。

完成本章，不只是把程式執行出來。你應能用自己的話說明原始碼如何經過編譯成為可執行檔，能在執行前預測結果，並能透過錯誤訊息與實際執行結果修正自己的理解。

本章活動不要求繳交。建議自行保留預測、程式、錯誤與修正結果，供之後複習或課堂討論使用。不使用 AI 不影響本章完成；任何 AI 相關活動均為可跳過的選用延伸。
...
| 類型 | 修改或操作 | 預期結果 |
|---|---|---|
| 正常案例 | 編譯並執行原始版本 | 出現 `Hello, C!` |
| 格式案例 | 移除最後的 `\n` | 終端機提示可能接在同一行 |
| 編譯錯誤 | 移除分號後編譯 | 編譯失敗，不產生對應的新版本 |
| 回歸確認 | 補回分號、重新編譯並執行 | 正確輸出恢復 |
| 舊版本實驗 | 修改文字但不重新編譯 | 仍執行舊可執行檔中的內容 |

這張表不是為了增加工作量，而是讓你在每次操作之前，先說清楚自己準備觀察什麼。
...
這一節可以直接跳過，不影響本章完成，也不需要任何未使用聲明。

若你選擇使用 AI，可以先用自己的話解釋：

> 原始碼、編譯器、可執行檔與程式執行之間有什麼關係？

不需要使用固定 Prompt，也不需要保存或繳交對話。AI 的回應仍可能不完整或錯誤；若它的說法和本章流程圖、編譯結果或實際執行現象不一致，請以可重現的操作與證據重新判斷，而不是直接把 AI 回應當成答案。
...
若其中某一項仍不確定，回到對應章節重新進行預測與實驗。

---

## 16. 本章摘要

C 原始碼是人類可以閱讀與修改的文字。編譯器讀取原始碼，成功時建立可執行檔。只有當作業系統啟動可執行檔後，程式才開始執行，並可能產生輸出。
...
下一個 Unit 將回答新的問題：程式執行時，如何保存資料並改變自己的狀態？
