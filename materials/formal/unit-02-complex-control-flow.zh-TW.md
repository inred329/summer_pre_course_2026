...
本章的核心完成標準不包含 AI 使用。未使用 AI 不影響本章完成、課堂參與或評量；文末的 AI 活動是可直接跳過的選用延伸。
...
這個活動可以直接跳過；不使用 AI 不影響本章完成、課堂參與或評量。若你選擇使用 AI，可以先用自己的話解釋：

> 多分支的順序為什麼重要？sentinel、EOF、無效輸入與 loop invariant 有什麼不同？

不需要使用固定 Prompt、保存或繳交對話，也不需要未使用聲明。AI 回應若與路徑表、讀取回傳規則、追蹤或可重現結果不一致，請以證據重新判斷。
