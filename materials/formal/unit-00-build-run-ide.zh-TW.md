...
你可能已經寫過程式，也可能已經習慣在 IDE 裡按 Run、Build 或 Debug。

這個 Unit 不會重新教你每一個按鈕在哪裡，而是先確認一件更重要的事：當你說「我把程式跑起來了」，中間其實經過了哪些不同步驟？

接下來正式課程會一直使用 compiler、Build、Run、Debug、測試與錯誤訊息。如果這些詞在腦中全部混成同一件事，後面很多問題都會看起來比實際上更神祕。

所以先從最熟悉的動作開始：

> 你按下 Run，IDE 到底替你做了什麼？
...
到這裡，我們先把工具層拆清楚了：source、Build、compiler、executable、Run、Debug 各自有不同角色。
