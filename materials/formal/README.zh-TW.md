...
如果你現在上的是正式課程，可以直接從 F-U01 開始，不需要先把前導課程 P-U01～P-U04 重新讀完。

**[開始 F-U01：表示、型別與運算為什麼會影響結果？](unit-01-representation-types.zh-TW.md)**

前導 Unit 是補充基礎的入口，不是正式班學生的必經閱讀路徑。如果你對編譯、變數與狀態、條件與迴圈、函數與參數等基礎概念不熟悉，可以再依需要回到前導教材複習。
...
請依 F-U01 至 F-U12 的順序閱讀。每個 Unit 結尾都有「下一單元」連結，因此開始 F-U01 後可以沿著教材一路往下，不需要回到 repository 首頁或其他內部文件尋找下一步。
...
如果其中某項不熟悉，再回到前導課程對應 Unit 複習即可，不需要把前導課程當成正式課程的固定前置閱讀清單。
...
- [開始 F-U01](unit-01-representation-types.zh-TW.md)
