...
AI 可以作為延伸工具，但不得因位於核心章節、完成清單、固定模板或評量欄位而成為隱性必做。所有已審查的前導 Unit、正式課程索引與 F-U01 至 F-U07 均已改為可直接跳過的選用延伸。
...
涉及陣列、字串、遞迴、指標與結構時，教材必須清楚交代值域、輸入狀態、終止條件、生命週期、可解參照範圍、字串容量、欄位驗證與未定義行為；成功編譯或單次正確輸出不能取代這些契約。
...
前導 Unit 1–4 與正式課程 F-U01 至 F-U07 目前均具備：
...
- `materials/formal/unit-08*` 至 `unit-12*`
- AI 是否仍位於核心完成主線。
...
目前已發現的 P0 問題均已修復。正式課程索引與 F-U01 至 F-U07 已完成審查；F-U07 已明確將 AI 活動移出核心完成路徑，並保留完整初始化、有效指標、有界字串、結構指定與欄位驗證契約。全庫審查尚未完成，下一批將處理 F-U08 至 F-U12、設計治理層級與自動驗證，因此 PR 仍維持 Draft。
