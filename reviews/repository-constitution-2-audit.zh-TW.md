...
| `design/02-requirements-map.*` | 可修訂的規劃與追蹤模型 | 課程設計者與維護者 | C → A | P1 | 將後續文件描述為受本需求基準控制；把 AI 素養與 AI 紀錄列為普遍需求；保留過時線性導覽 | 明定本文件非治理來源且受上位規範約束；外部協助驗證改為條件式；取消 AI 必用與必留紀錄；改為維護導覽 | 已完成 |
| `design/03-programming-domain-model.*` | 可修訂的規劃模型 | 課程設計者與維護者 | C → A | P1 | 將 AI 嵌入程式核心領域、要求透明 AI 紀錄、雙語 metadata 漂移，且把實體建置流程描述得過於絕對 | AI 改為選用外部協助；揭露改為情境式；同步版本與狀態；加入不綁定實作的工具鏈例外並修正指標／儲存模型文字 | 已完成 |
| `design/04-programming-language-knowledge-graph.*` | 可修訂的規劃與追蹤模型 | 課程設計者與維護者 | C → A | P1 | 把 AI 判斷／驗證列為普遍橫向依賴、要求 AI 紀錄路徑、保留草案／下一階段導覽，並過度具體化前置處理／組譯／連結產物 | 新增條件式依賴；外部協助分支改為選用；取消普遍紀錄要求；同步 metadata；改為相關文件導覽；釐清概念建置階段、實作差異與 runtime 模型 | 已完成 |

## 三、主要裁決

### 1. AI 活動不得形成隱性必做

AI 可以作為延伸工具，但不得因位於核心章節、完成清單、固定模板、需求基準、領域模型、知識依賴圖、能力欄位或評量欄位而成為隱性必做。不使用 AI 不影響完成、參與或評量。規劃模型只有在學生實際採用選用外部協助時，才能條件式描述其理解與驗證責任。

### 2. 評量規則維持單一權威來源

`design/13-learning-assessment-policy.*` 是正式評量制度的唯一權威來源。README、作業包、rubric、模板、教師指南與規劃模型只能提供導覽、追蹤或執行方式。
...
教材必須交代相關值域、輸入狀態、終止條件、生命週期、解參考前提、配置大小、所有權、失敗保留、stream 狀態、模組依賴、編譯／連結差異與未定義行為。成功編譯或單次正確輸出不能取代這些契約。

### 5. 概念模型不得被誤認為實作保證

設計圖可簡化語言工具鏈或 runtime 責任，但必須標示實作相依細節。C 標準並不要求固定的一套獨立 preprocessor、compiler、assembler、linker，也不要求唯一的實體呼叫堆疊表示。當具體差異會影響教材時，必須指出並驗證指定編譯器與執行環境。
...
`design/01–04` 現在已符合此分類與 AI 選用政策。下一批設計治理工作繼續逐份審查 `design/05–11` 與 `design/14–17`，確認是否仍有重複規範宣告、過時 metadata、過時「下一階段」文字、工具特定假設或雙語實質不等值。

## 六、下一批審查範圍

### P2：個別設計文件

逐份審查 `design/05–11` 與 `design/14–17`：

- 是否宣稱與治理層級不符的權威，或重複正式政策。
- 是否存在未經正式政策授權的 AI／工具必用要求。
- 是否保留過時草案狀態或線性「下一階段」文字。
- 中英文版本、狀態與核心內容是否一致。
- 技術敘述是否把概念模型誤寫成 C 標準或特定實作保證。
...
目前已發現的 P0 問題均已修復。前導教材與正式課程 F-U01 至 F-U12 已完成雙語審查，`design/01–04` 的雙語 P1 問題也已修復並提交。全庫審查尚未完成；仍需處理 `design/05–11`、`design/14–17` 與自動化導覽／技術驗證，因此 PR #10 繼續維持 Draft。
