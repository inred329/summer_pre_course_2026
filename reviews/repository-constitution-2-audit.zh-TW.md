...
| `materials/formal/unit-03-arrays.*` | Student Material | 學生 | B → A | P1 | AI 解釋位於核心主線；累加範例未聲明有號整數溢位前提；成績練習未要求驗證輸入格式與範圍 | AI 改為可跳過延伸；補上非 AI 完成標準、累加可表示範圍、輸入驗證與空集合契約 | 已完成 |

## 三、主要裁決

### 1. AI 活動的學生教材位置

AI 可以作為延伸工具，但不得因位於核心章節主線、完成清單、模板固定欄位或評量欄位而形成隱性必做。所有已審查的前導 Unit、正式課程索引、F-U01、F-U02、F-U03 與教材模板均已改為清楚標示、可直接跳過的選用延伸。
...
陣列走訪正確不代表累加一定安全。教材若以 `int` 累加，必須明確限制元素與總和範圍，或改用可表示資料的型別並檢查溢位。輸入資料必須先驗證格式與值域，空集合或只含一個元素的需求也必須有明確契約。

## 四、已確認的學生教材品質

前導 Unit 1–4 與正式課程 F-U01、F-U02、F-U03 目前均具備：
...
- `materials/formal/unit-04*` 至 `unit-12*`
- 是否仍把 AI 活動放入核心完成主線。
...
目前已發現的 P0 問題均已修復。正式課程索引與 F-U01 至 F-U03 已完成審查，F-U03 的 AI 定位、整數累加範圍、輸入驗證與空集合契約問題已修正。全庫審查尚未完成，下一批將繼續處理 F-U04 至 F-U12、設計治理層級與自動驗證，因此 PR 仍維持 Draft。