...
| `materials/formal/unit-08-dynamic-memory.*` | Student Material | Students | B → A | P1 | AI remained in the core path; allocation-size overflow, zero-size growth, alias lifetime, and `realloc` metadata contracts were incomplete | Made AI optional; added count-to-byte overflow checks, explicit zero-capacity handling, alias invalidation, and failure-preserving growth contracts | Completed |
| `materials/formal/unit-09-files.*` | Student Material | Students | B → A | P1 | AI remained in the core path; line-based input mentioned truncation but did not define a detection and response contract | Made AI directly skippable; added newline/EOF truncation detection, fragment handling, malformed/overlong tests, and checked I/O contracts | Completed |

## 3. Major Decisions

### 3.1 AI activities must not become implicitly required

AI may be offered as an extension, but placement in a core chapter, completion list, fixed template, or assessment field must not turn it into required work. All reviewed preparatory Units, the formal-course index, and F-U01 through F-U09 now make AI directly skippable.
...
Materials involving arrays, strings, recursion, pointers, structures, dynamic memory, and files must state value ranges, input state, termination conditions, lifetime, dereference boundaries, allocation-size calculations, ownership, release responsibility, failure preservation, stream state, format failure, buffer fragmentation, and undefined-behavior risks. Compilation success or one correct output cannot replace these contracts.
...
Preparatory Units 1–4 and formal-course F-U01 through F-U09 now provide:

- Problem context before syntax.
- Prediction and state, flow, memory, or stream tracing.
...
- `materials/formal/unit-10*` through `unit-12*`
...
All P0 issues identified so far have been corrected. The formal-course index and F-U01 through F-U09 have been reviewed. F-U09 now places AI outside the core path and states operational contracts for checked open/read/write/close behavior, EOF versus format and I/O failure, overlong-line detection, buffer fragments, and malformed-file tests. The repository-wide audit is not complete; the next batch continues with F-U10 through F-U12, design governance, and automated validation, so the pull request remains a draft.